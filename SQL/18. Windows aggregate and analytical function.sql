...
select distinct student_batch from inst_students;

select distinct student_name from inst_students;

select count(distinct student_batch) from inst_students;

select count(distinct students_marks) from inst_students;

select count(*) from inst_students group by students_marks;

select students_batch, sum(students_marks), count(*) from inst_students group by students_batch;
...
select * from inst_students order by students_marks desc limit 6 , 3 ;
...
JOIN 
( SELECT student_batch, MAX(students_marks) AS max_marks FROM inst_students GROUP BY student_batch)
